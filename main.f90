...
!Change by Nick Lutsko April 1st 2024
use params
implicit none
...
!Change by Nick Lutsko 1st April 2024
!Add nudging of RH
!thresh_rh and tau_qs are new parameters

      do k=1,nzm
       if (pres(k).ge.100.) then
          ...
          do j=1,ny
             do i=1,nx
              ...
              if (qv(i,j,k)/qsat.le.thresh_rh) then
                 qv(i,j,k)=qv(i,j,k)+dtn* (thresh_rh*qsat - qv(i,j,k))/tau_qs
                 !add water vapor to total water as well, which is prognostic variable
                 q(i,j,k)=q(i,j,k)+dtn*(thresh_rh*qsat - qv(i,j,k))/tau_qs
              endif
           end do
          end do
        end if
